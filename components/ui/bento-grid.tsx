...
const BentoGrid = ({ children, className, ...props }: BentoGridProps) => {
  return (
    <div
      className={cn(
        "grid w-full auto-rows-[22rem] grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4",
        className
      )}
      {...props}
    >
...
const BentoCard = ({
  ...
  >
    ...
    {/* Blur overlay - absolute positioned, edge to edge with gradient */}
    {blur && (
      <div 
    ...
    <div className="p-4 relative z-10">
      <div className="pointer-events-none flex transform-gpu flex-col gap-1 transition-all duration-300 lg:group-hover:-translate-y-10">
        <Icon className="h-12 w-12 origin-left transform-gpu text-primary dark:text-[#F37272] transition-all duration-300 ease-in-out group-hover:scale-75" />
        <h3 className={cn(
          "text-xl font-semibold",
          blur ? "text-neutral-100 dark:text-neutral-300" : "text-neutral-700 dark:text-neutral-300"
        )}>
          {name}
        </h3>
        <p className={cn(
          "max-w-lg",
          blur ? "text-neutral-300" : "text-neutral-400"
        )}>{description}</p>
      </div>
